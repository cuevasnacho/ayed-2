...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    unsigned int array_year;
    unsigned int array_month;
    unsigned int array_day;
    ...
    while (!feof(file)) {
        ...
        array_year = k_year - 1980;
        array_month = k_month - 1;
        array_day = k_day - 1;

        Weather weather = weather_from_file(file);
        array[array_year][array_month][array_day] = weather;
    }
    fclose(file);
}
