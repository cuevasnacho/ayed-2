...
static bool invrep(abb tree) {
    ...
    if (tree!=NULL) {
        if (tree->left!=NULL) {
            b = tree->elem > tree->left->elem && invrep(tree->left);
        }
        if (tree->right!=NULL) {
            b = b && (tree->elem < tree->right->elem) && invrep(tree->right);
        }
    }
    return b;
}

static struct _s_abb *create_leaf(abb_elem e) {
    struct _s_abb *new_leaf=NULL;
    new_leaf = malloc(sizeof(struct _s_abb));
    new_leaf->elem = e;
    new_leaf->left = NULL;
    new_leaf->right = NULL;
    return new_leaf;
